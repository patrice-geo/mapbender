...
class WMSController extends Controller
{
    ...
    public function addAction()
    {
        ...
        $wms->setSupportedExceptionFormats(
            isset($requestWMS['exceptionFormats']) ? $requestWMS['exceptionFormats'] : array());

        $wms->setRequestSupportedGetCapabilitiesFormats(
            isset($requestWMS['requestGetCapabilitiesFormats']) ? $requestWMS['requestGetCapabilitiesFormats'] : array());

        $wms->setRequestSupportedGetMapFormats(
            isset($requestWMS['requestGetMapFormats']) ? $requestWMS['requestGetMapFormats'] : array());

        $wms->setRequestSupportedGetFeatureInfoFormats(
            isset($requestWMS['requestGetFeatureInfoFormats']) ? $requestWMS['requestGetFeatureInfoFormats'] : array());

        $wms->setRequestSupportedDescribeLayerFormats(
            isset($requestWMS['requestDescribeLayerFormats']) ? $requestWMS['requestDescribeLayerFormats'] : array());

        $wms->setRequestSupportedGetLegendGraphicFormats(
            isset($requestWMS['requestGetLegendGraphicFormats']) ? $requestWMS['requestGetLegendGraphicFormats'] : array());

        $wms->setRequestSupportedGetStylesFormats(
            isset($requestWMS['requestGetStylesFormats']) ? $requestWMS['requestGetStylesFormats'] : array());

        $wms->setRequestSupportedPutStylesFormats(
            isset($requestWMS['requestPutStylesFormats']) ? $requestWMS['requestPutStylesFormats'] : array());
        ...
        if ($form->isValid()) {
            ...
            return $this->render("MapbenderWmsBundle:WMS:preview.html.twig",
                    array(
                    "getcapa_url" => "",
                    "wms" => $wms,
                    "form" => $form->createView(),
    ...
    public function saveAction(WMSService $wms)
    {
        ...
        if ($form->isValid()) {
            ...
            return $this->render("MapbenderWmsBundle:WMS:edit.html.twig",
                    array(
                    "wms" => $wms,
                    "form" => $form->createView(),
            ));
    ...
    public function updateAction(WMSService $wms)
    {
        ...
        $newForm = $this->get('form.factory')->create(new WMSType(), $newWms,
            array(
    ...
    public function deleteAction(WMSService $wms)
    {
        ...
        if ($wms->getId() === null) {
            try { // remove monitoringdefinition if exists
                ...
                $response = $this->forward(
                    "MapbenderMonitoringBundle:MonitoringDefinition:fromwmsdelete", array("mdId" => $md->getId()));
            } catch (\Exception $e) {
